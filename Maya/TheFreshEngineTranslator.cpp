#include "TheFreshEngineTranslator.h"
...
MStatus TheFreshEngineTranslator::writer(const MFileObject& fileObject,const MString& optionsString, MPxFileTranslator::FileAccessMode mode)
{
	uint objectCount = 0;
	MStatus status;
...
bool TheFreshEngineTranslator::haveWriteMethod () const
{
	return true;
}
bool TheFreshEngineTranslator::haveReadMethod () const
{
	return false;
}
bool TheFreshEngineTranslator::canBeOpened () const
{
	return false;
}

MString TheFreshEngineTranslator::defaultExtension () const
{
	return "frh";
}

void* TheFreshEngineTranslator::creator()
{
	return new TheFreshEngineTranslator();
}
...
//			MStatus::kFailure otherwise
{
	...
	status =  plugin.registerFileTranslator("The Fresh Engine",
											"",
											TheFreshEngineTranslator::creator,
											"",
											"option1=1",
											true);
	if (!status) {
